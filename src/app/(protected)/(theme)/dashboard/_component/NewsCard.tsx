...
async function NewsCard() {
  ...
  if (
    ...
    const response = await fetch(
      `https://gnews.io/api/v4/search?q=tech&lang=en&country=in&max=5&apikey=${process.env.NEWS_API_KEY}`,
    ).then((res) => res.json())
  ...
  const articleResponse = articles.chain().simplesort('$loki', true).data()
